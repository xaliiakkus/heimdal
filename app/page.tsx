        ...
        <div className="p-6">
          ...
          <div className="border border-[#a2a9b1] bg-[#f8f9fa] p-4 my-6 inline-block min-w-[350px]">
            ...
            <ul className="list-none ml-4">
              {["Tarihçe", "Başarılar ve Turnuvalar", "Ünlü Futbolcuları", "Teknik Direktörler", "Unutulmaz Maçlar", "İstatistikler"].map((item, i) => (
                <li key={item} className="my-1.5">
                  <a href={`#${item.toLowerCase().replace(/\s/g, "-").replace("ı", "i").replace("ş", "s").replace("ö", "o").replace("ü", "u").replace("ç", "c").replace("ğ", "g")}`} className="text-[#0645ad] no-underline hover:underline">
                    {i + 1}. {item}
                  </a>
                </li>
              ))}
